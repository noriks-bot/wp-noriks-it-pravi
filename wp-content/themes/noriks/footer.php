...
<!-- suport section -->
	...
	if ( ! $nf_brand ) {
		$nf_brand = 'NORIKS è un marchio europeo di prodotti per l’uso quotidiano. Ci concentriamo su qualità, comfort e prezzi onesti — più di 120.000 clienti si fidano già di noi.';
	}
	$nf_bg      = get_field( 'footer_bg_image', 'option' ); // background photo behind the brand band
	$nf_legal   = get_field( 'footer_legal_text', 'option' );
	if ( ! $nf_legal ) {
		$nf_legal = 'NORIKS BRAND. Tutti i diritti riservati. I prezzi includono l\'IVA. Le immagini hanno carattere illustrativo.';
	}
	?>
		...
		<section class="nf-signup">
			<div class="nf-wrap nf-signup-grid">
				<div class="nf-signup-copy">
					<h2 class="nf-signup-title">Unisciti alla famiglia NORIKS</h2>
					<p class="nf-signup-sub">Offerte esclusive e anteprime dei nuovi prodotti — scoprile per primo.</p>
				</div>
				<div class="nf-signup-form-col">
					<form class="nf-signup-form" data-klaviyo-list="<?php echo esc_attr( $nf_klaviyo_list ); ?>" novalidate>
						<label class="nf-field-label" for="nf-email">La tua e-mail *</label>
						<div class="nf-field-row">
							<input type="email" id="nf-email" name="email" class="nf-input" placeholder="Il tuo indirizzo email" required autocomplete="email">
							<button type="submit" class="nf-btn nf-btn-light">Iscriviti</button>
						</div>
						<p class="nf-signup-note">
							Iscrivendoti accetti di ricevere e-mail. Puoi disiscriverti in qualsiasi momento.
		...
		<div class="nf-wrap nf-meta">
			<div class="nf-meta-left">
				<span class="nf-region">
					<img loading="lazy" decoding="async" src="https://static.devit.software/countries/flags/rectangle/it.svg" alt="" class="nf-flag">
					Italia (IT)
				</span>
				<span class="nf-copy">© <?php echo date( 'Y' ); ?> NORIKS. Tutti i diritti riservati.</span>
			</div>
	...
	(function () {
		...
		form.addEventListener('submit', function (e) {
			...
			if (!email || email.indexOf('@') === -1) {
				msg.textContent = 'Inserisci un indirizzo e-mail valido.'; msg.className = 'nf-signup-msg is-error'; return;
			}
			if (!list || list === 'REPLACE_LIST_ID') {
				msg.textContent = 'La newsletter non è ancora collegata (manca il Klaviyo List ID).'; msg.className = 'nf-signup-msg is-error'; return;
			}
			msg.textContent = 'Iscrizione in corso…'; msg.className = 'nf-signup-msg';
			...
			fetch('https://manage.kmail-lists.com/ajax/subscriptions/subscribe', {
				...
				if (d && d.success) {
					msg.textContent = 'Grazie! Controlla la tua e-mail per la conferma.'; msg.className = 'nf-signup-msg is-ok';
					form.reset();
				} else {
					msg.textContent = (d && d.errors && d.errors[0]) ? d.errors[0] : 'Qualcosa è andato storto, riprova.';
					msg.className = 'nf-signup-msg is-error';
				}
			}).catch(function () {
				msg.textContent = 'Qualcosa è andato storto, riprova.'; msg.className = 'nf-signup-msg is-error';
			});
		});
	})();
	</script>
