	...
	acf_add_local_field_group( array(
		...
		'fields' => array(
			...
			array(
				...
				'label'        => 'Mostra upsell #3 sotto il pulsante (maglietta compressiva NORIKS FIT)',
				'name'         => 'noriks_pp_upsell3',
				'type'         => 'true_false',
				'instructions' => 'Terzo riquadro upsell: maglietta compressiva NORIKS FIT, il cliente sceglie la taglia. Indipendente dai primi due interruttori.',
				'ui'           => 1,
			),
		),
		'location'   => array(
...
function noriks_pp_upsell_render() {
	...
	?>
	<div class="npu-wrap">
		<span class="npu-label">Acquista insieme e risparmia:</span>
			...
			<div class="npu-grid">
				...
				<div class="npu-actions">
					<label class="npu-check">
						...
						<span class="npu-check-text">Aggiungi all'acquisto</span>
					</label>

					<select class="npu-size" name="noriks_pp_upsell_size" aria-label="Taglia dei boxer">
...
function noriks_pp_upsell2_config() {
	return apply_filters( 'noriks_pp_upsell2_config', array(
		...
		'title'      => '1x Maglietta nera + 1x grigia',
		'desc'       => 'Due magliette basic nel pacchetto — aggiungile al tuo ordine con il %s%% di sconto.',
		'size_attr'  => 'Taglia',
		'sku'        => 'NORIKS-SHIRTS-BLACK-GRAY-2-PACK-UPSELL',
		'image'      => get_template_directory_uri() . '/img/upsell/upsell-2x-majici.png',
		'label'      => 'Acquista insieme e risparmia:',
		'add_text'   => 'Aggiungi all\'acquisto',
		'aria'       => 'Taglia delle magliette',
	) );
}
...
function noriks_pp_upsell2_maybe_add( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
	...
	$lines  = array(
		( $prod_a ? $prod_a->get_name() : 'Maglietta nera' ) . ' - ' . $size,
		( $prod_b ? $prod_b->get_name() : 'grigia' ) . ' - ' . $size,
	);
...
function noriks_pp_upsell3_config() {
	return apply_filters( 'noriks_pp_upsell3_config', array(
		'product_id' => 7663,   // NORIKS FIT kompresijska majica (jednostavan proizvod)
		'total'      => 17.99,  // cijena za 1 komad
		'title'      => 'Maglietta compressiva NORIKS FIT',
		'desc'       => 'Modella la silhouette e sostiene la postura — aggiungila al tuo ordine con il %s%% di sconto.',
		'size_attr'  => 'Taglia',
...
function noriks_pp_upsell3_order_item_meta( $item, $cart_item_key, $values, $order ) {
	if ( ! empty( $values['_noriks_pp_upsell3'] ) ) {
		...
		if ( ! empty( $values['_noriks_pp_upsell_size'] ) ) {
			$item->update_meta_data( 'Taglia', sanitize_text_field( $values['_noriks_pp_upsell_size'] ) );
		}
	}
}
