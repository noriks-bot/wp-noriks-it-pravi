...
function noriks_remove_upsell() {
    ...
    if ( $item->get_meta( '_noriks_upsell' ) !== 'thank you upsell' ) {
        wp_send_json_error( 'Solo i prodotti upsell possono essere rimossi' );
    }
...
function noriks_handle_add_upsell() {
    ...
    if ( $order->get_payment_method() !== 'cod' ) {
        wp_send_json_error( 'Upsell disponibile solo per contrassegno' );
    }
    if ( $order->get_status() !== 'primary-hold' ) {
    ...
    if ( ! $active_price ) {
        wp_send_json_error( 'Il prezzo del prodotto non è disponibile' );
    }
    ...
    $is_majice = ( strpos($cat_str, 'orto-magliett') !== false || strpos($cat_str, 'magliett') !== false || strpos($name, 'magliett') !== false || strpos($cat_str, 'majic') !== false || strpos($name, 'majic') !== false );
    ...
    $order->add_order_note(
        sprintf(
            'Thank you upsell: %s aggiunto con 50%% di sconto — prezzo offerta: %s, prezzo upsell: %s',
            $product->get_name(),
