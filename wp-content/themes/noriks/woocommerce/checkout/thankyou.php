...
foreach ( $order->get_items() as $item ) {
    ...
    $is_majica = ( strpos($cat_str, 'orto-magliett') !== false || strpos($cat_str, 'magliett') !== false || strpos($name, 'magliett') !== false || strpos($cat_str, 'majic') !== false || strpos($name, 'majic') !== false );
    ...
    // Komplet
    $is_komplet = ( strpos($cat_str, 'set') !== false || strpos($cat_str, 'orto-starter') !== false || strpos($cat_str, 'orto-maglietta-boxer') !== false || strpos($name, 'komplet') !== false || strpos($cat_str, 'komplet') !== false );
...
if ( $upsell_is_majice ) {
    ...
    $upsell_name       = 'Magliette nere';
    $upsell_qty_prices = array( 1 => 12.99, 3 => 29.99, 6 => 39.99 );
    $upsell_qty_names  = array( 1 => '1x Maglietta nera', 3 => '3x Magliette nere', 6 => '6x Magliette nere' );
...
<div class="ty-container">
    ...
    <?php else : ?>
        ...
        <div class="ty-success">
            ...
            <p>La conferma è stata inviata a <?php echo esc_html( $order->get_billing_email() ); ?></p>
            <span class="ty-order-num">Ordine #<?php echo $order->get_order_number(); ?></span>
        </div>
                ...
                <div class="tyuo_product_section">
                    ...
                    <div class="ty-qty-picker" style="display:flex;gap:8px;padding:0 0 10px;justify-content:center;">
                        ...
                        <label class="ty-qty-btn<?php echo $cls; ?>" style="flex:1;text-align:center;padding:10px 0;border:2px solid <?php echo $border; ?>;border-radius:4px;font-weight:700;font-size:14px;cursor:pointer;background:<?php echo $bg; ?>;color:#000;">
                            <input type="radio" name="ty_qty" value="<?php echo $q; ?>"<?php echo $chk; ?> style="display:none;"> <?php echo $q; ?>x pz
                        </label>
                        <?php endforeach; ?>
                    </div>
        ...
        <div class="ty-section" id="ty-order-items-section">
            <div class="ty-section-header open" onclick="tyToggle(this)">
                <span id="ty-order-items-header">Articoli dell'ordine (<?php echo $order->get_item_count(); ?>)</span>
...
<?php else : ?>
    <div class="ty-container">
        <div class="ty-success"><h1>Ordine</h1>
...
(function(){
    ...
    function tick() {
        if (rem <= 0) {
            ...
            // Auto-open "Articoli dell'ordine" and "Indirizzo di spedizione" sections
    ...
    // ─── Step 1: "AGGIUNGI" → add to order, then show grid ───
    ...
    if (overlay) {
        ...
        if (gridAddAll) {
            gridAddAll.addEventListener('click', function() {
                ...
                gridAddAll.textContent = 'Aggiungo...';
