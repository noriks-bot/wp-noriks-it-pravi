...
function gck_it_majice_phrase( int $n, bool $free = false, string $type = 'majica' ) : string {
    if ( $type === 'bokserica' ) {
        $noun = 'boxer';
        return $free ? ( $noun . ' gratis' ) : $noun;
    }
    if ( $type === 'carapa' ) {
        $noun = ( $n === 1 ) ? 'calza' : 'calze';
        return $free ? ( $noun . ' gratis' ) : $noun;
    }
...
function gck_render_bundle_selector() {
    ...
    if (
        ...
        || ( stripos( (string) $product->get_slug(), 'calzin' ) !== false )
        || ( stripos( (string) $product->get_slug(), 'calze' ) !== false )
        || ( stripos( (string) $product->get_name(), 'calzin' ) !== false )
        || ( stripos( (string) $product->get_name(), 'calze' ) !== false )
    ) {
    ...
    //  - orto-ortopas                 : single "Taglia" attribute, no colour (size selector only).
    ...
    // extended to 3 garment groups (4 magliette + 1 boxer + 1 calza). Gated
    // strictly to this category. This site's save/sync collapses product
    // attributes to a single Colore/Taglia, so the 3 groups' colour/size options
  ...
  margin-right:6px;
    ...
    <?php
    if (  !has_term( array( 'orto-starter', 'orto-majice', 'orto-magliette', 'orto-bokserice', 'orto-boxer', 'orto-kompresijske-carape', 'orto-ortopas', 'orto-bunion', 'orto-fisiorest', 'orto-norikshers', 'orto-noriks-hers', 'orto-majica-darila', 'orto-leak-boxers', 'orto-kompresijske-majice', 'orto-ortopedski-jastuk', 'orto-kidsnest' ), 'product_cat', $product_id )  )   :
    ?>
        <style>
    ...
    <?php
    // SHGIFTS: the calza size labels ("39-43" …) are wider than shirt/boxer sizes,
    ...
    <div class="gck-benefits-box">
        ...
        <?php if ( ! $show_countdown && ! $gck_no_attrs && ! $gck_single_size && ! $gck_shgifts && ! has_term( array( 'orto-leak-boxers', 'orto-kompresijske-majice' ), 'product_cat', $product_id ) ) : // no size chart on the gifts set; leak boxers + kompresijske majice show it above the bundle buttons instead ?>
    ...
    <div id="bundle-selector" class="bundle-box<?php echo $gck_single_size ? ' is-single-size' : ''; ?><?php echo $gck_no_attrs ? ' is-no-attrs' : ''; ?>" data-split-garments="<?php echo $gck_split_garments ? '1' : '0'; ?>">
        ...
        foreach ( $offers as $offer_id => $data ) :
            ...
            // SHGIFTS: real piece count is 6 (4 magliette + 1 boxer + 1 calza),
    ...
    && !has_term( array( 'starter-paketi' ), 'product_cat', $product_id ) 
                ...
                <?php if ( $show_price_highlights ) : ?>
                    ...
                    <span class="gck-per-chip">
                        ...
                        <?php endif; ?>
                        <span class="gck-per-new"><?php echo number_format( $gck_per_new, 2 ); ?>€ / pz</span>
                    </span>
                    <?php if ( $discount_pct > 0 ) : ?>
                        <span class="gck-discount-badge">−<?php echo (int) $discount_pct; ?>%</span>
                    <?php endif; ?>
                <?php else : ?>
                    — <span class="bundle-option-title"><?php echo number_format( $gck_per_new, 2 ); ?>€ / pz</span>
                <?php endif; ?>
                <?php endif; ?>
...
<div class="bundle-total-line">
    ...
    <span class="line-total"><?php echo number_format( (float) $data['total'], 2 ); ?>€</span>
                ...
                <div class="bundle-pairs <?php echo $is_default ? '' : 'hidden'; ?>"
                    ...
                    // attributes — "Colore maglietta"/"Taglia maglietta", "Colore boxer"/
                    // "Taglia boxer", "Colore calza"/"Taglia calza" — grouped by the garment
                    // word via the same helper SHBOX uses ($attr_groups). Form field names
                    // stay stable (majica_color …) so cart/label capture and the socks-width
                    // CSS are unaffected. Falls back to hardcoded options if a garment group
                    // can't be resolved, so it never breaks. Each item gets its own pair
                    // index so all 6 selections reach the cart.
                    $gck_sections = array(
                        array(
                            'label'   => 'Scegli 4 magliette',
                            'gratis'  => false, 'count' => 4, 'gg' => 0, 'garment' => 'majica',
                            'ckey'    => 'majica_color', 'skey' => 'majica_size',
                            'colors'  => array( 'Nero', 'Bianco', 'Grigio', 'Beige', 'Blu scuro', 'Marrone', 'Verde' ),
                            'sizes'   => array( 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL' ),
                        ),
                        array(
                            'label'   => 'Scegli 1 boxer in omaggio',
                            'gratis'  => true, 'count' => 1, 'gg' => 1, 'garment' => 'bokserica',
                            'ckey'    => 'bokserica_color', 'skey' => 'bokserica_size',
                            'colors'  => array( 'Nero', 'Blu', 'Grigio', 'Verde', 'Rosso' ),
                            'sizes'   => array( 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL' ),
                        ),
                        array(
                            'label'   => 'Scegli il colore delle calze (5 paia)',
                            'gratis'  => true, 'count' => 1, 'gg' => 2, 'garment' => 'carapa',
                            'ckey'    => 'carapa_color', 'skey' => 'carapa_size',
                            'colors'  => array( 'Nero', 'Bianco' ),
                    ...
                    foreach ( (array) $attr_groups as $grp ) {
                        ...
                        if ( strpos( $hay, 'majic' ) !== false || strpos( $hay, 'maglie' ) !== false )       { $gck_real['majica']    = array( 'colors' => $cv, 'sizes' => $sv ); }
                        elseif ( strpos( $hay, 'bokseric' ) !== false || strpos( $hay, 'boxer' ) !== false ) { $gck_real['bokserica'] = array( 'colors' => $cv, 'sizes' => $sv ); }
                        elseif ( strpos( $hay, 'carap' ) !== false || strpos( $hay, 'calz' ) !== false )     { $gck_real['carapa']    = array( 'colors' => $cv, 'sizes' => $sv ); }
                    }
                    ...
                    if ( $gck_split_garments ) {
                        $gck_passes = array(
                            array( 'group' => 0, 'label' => 'Scegli ' . (int) $pairs . ' ' . gck_it_majice_phrase( $pairs, false, 'majica' ), 'gratis' => false ),
                            array( 'group' => 1, 'label' => 'Scegli altri ' . (int) $pairs . ' ' . gck_it_majice_phrase( $pairs, true, 'bokserica' ), 'gratis' => true ),
                        );
                    } else {
                    ...
                    <?php for ( $i = 1; $i <= $pairs; $i++ ) : ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_paid > 0 && $i === 1 ) : ?>
                            <div class="gck-pair-label">Scegli <?php echo (int) $gck_paid; ?> <?php echo esc_html( gck_it_majice_phrase( $gck_paid, false, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_free > 0 && $i === ( $gck_paid + 1 ) ) : ?>
                            <div class="gck-pair-label is-gratis">Scegli altre <?php echo (int) $gck_free; ?> <?php echo esc_html( gck_it_majice_phrase( $gck_free, true, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <div class="bundle-pair">
                    ...
                    <?php if ( ! $gck_no_attrs ) : ?>
                    <small style="display: block; line-height: 1;"><?php esc_html_e( 'Offriamo 14 giorni per il rimborso o cambio gratuito del prodotto – acquisti senza pensieri!', 'gift-card-kompetentnost' ); ?></small>
                    <?php endif; ?>
                </div>
            </label>
        <?php
 ...
 * total: maglietta 1-4 (colour+size), boxer (colour+size), calza (colour+size).
 * Each line is labelled by inspecting its field keys so the cart / order clearly
 * shows which garment was picked, e.g. "Maglietta: Nero - L", "Boxer: Blu - M",
 * "Calza: Nero - 39-43".
 */
function gck_build_shgifts_lines( array $pairs_data ) : array {
    ...
    foreach ( $pairs_data as $attrs ) {
        ...
        foreach ( array_keys( $attrs ) as $k ) {
            ...
            if ( strpos( $kl, 'majic' ) !== false )    { $label = 'Maglietta'; break; }
            if ( strpos( $kl, 'bokseric' ) !== false ) { $label = 'Boxer';     break; }
            if ( strpos( $kl, 'carap' ) !== false )    { $label = 'Calza';     break; }
        }
...
function gck_add_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
    ...
    // SHGIFTS (orto-majica-darila): 6 items (4 magliette + 1 boxer + 1 calza),
...
function gck_shgifts_free_gifts_notice() {
    ...
    // Gifts shown: 1x boxer + 5x paio di calze. Images + crossed-out reference
    ...
    $gifts = array(
        array( 'label' => '1x boxer', 'img' => $boxer_img, 'price' => '19,99 €' ),
    );
    for ( $i = 0; $i < 5; $i++ ) {
        $gifts[] = array( 'label' => '1x paio di calze', 'img' => $socks_img, 'price' => '12,99 €' );
    }
    ?>
    <style>
      .gck-fg{
    ...
    <div class="gck-fg">
        <div class="gck-fg__head">6 prodotti GRATIS con ogni acquisto</div>
        <div class="gck-fg__grid">
            <?php foreach ( $gifts as $g ) : ?>
                <div class="gck-fg__card">
                    <span class="gck-fg__badge">Omaggio</span>
